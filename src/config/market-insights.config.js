module.exports = {
  ...
  marketInsights: {
    ...
    cron: {
      schedule: process.env.MARKET_INSIGHTS_CRON || '*/1 * * * *', // Every minute by default
  ...
  screens: [
    ...
    },
    {
      ...
      cron: {
        schedule: process.env.TOP_LOSERS_CRON || '*/1 * * * *',
        timezone: process.env.CRON_TIMEZONE || 'Asia/Kolkata'
      }
    },
    {
      ...
      cron: {
        schedule: process.env.NEW_52W_LOW_CRON || '*/1 * * * *', // Every 15 minutes by default
        timezone: process.env.CRON_TIMEZONE || 'Asia/Kolkata'
      }
    },
    {
      ...
      cron: {
        schedule: process.env.RELATIVE_OUTPERFORMANCE_CRON || '*/1 * * * *', // Every hour by default
        timezone: process.env.CRON_TIMEZONE || 'Asia/Kolkata'
      }
    },
    {
      ...
      cron: {
        schedule: process.env.RELATIVE_UNDERPERFORMANCE_CRON || '*/1 * * * *',
        timezone: process.env.CRON_TIMEZONE || 'Asia/Kolkata'
      }
    },
    {
      ...
      cron: {
        schedule: process.env.HIGH_VOLUME_LOSERS_CRON || '*/1 * * * *', // Every 10 minutes by default
        timezone: process.env.CRON_TIMEZONE || 'Asia/Kolkata'
      }
    }
  ],
